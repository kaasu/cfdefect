...
<cfset SecurityController.validateUser( event ) >
<cfif event.getValue( 'isValid', 'false' )>
<cflocation url="#myFusebox.getMyself()##xfa.home#" addtoken="false">
<cfabort>
<cfelse>
<!--- do action="login" --->
<cfset myFusebox.thisFuseaction = "login">
<cfset xfa.dologin = "c.doLogin" />
<cftry>
<cfsavecontent variable="content.pageContent"><cfoutput><cfinclude template="../views/dsp_login.cfm"></cfoutput></cfsavecontent>
...
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse dsp_login.cfm in circuit v which does not exist (from fuseaction c.login).">
