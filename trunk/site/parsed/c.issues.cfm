...
<cfset myFusebox.thisFuseaction = "issues">
...
<cfset xfa.add = "c.editIssue&projectidfk=#event.getValue( 'projectidfk' )#" />
<cfset xfa.delete = "c.deleteIssue&projectidfk=#event.getValue( 'projectidfk' )#" />
<cfset xfa.edit = "c.editIssue&projectidfk=#event.getValue( 'projectidfk' )#" />
<cfset Controller.getProjectName( event ) >
<cfset Controller.getIssueHeader( event ) >
<cfset Controller.getIssueSupportingData( event ) >
<cftry>
<cfsavecontent variable="content.pageContent"><cfoutput><cfinclude template="../views/dsp_issues.cfm"></cfoutput></cfsavecontent>
<cfcatch type="missingInclude"><cfif len(cfcatch.MissingFileName) gte 14 and right(cfcatch.MissingFileName,14) is "dsp_issues.cfm">
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse dsp_issues.cfm in circuit v which does not exist (from fuseaction c.issues).">
