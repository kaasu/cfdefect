...
<cfcomponent hint="I am the database agnostic custom Record object for the users object.  I am generated, but not overwritten if I exist.  You are safe to edit me."
	extends="reactor.project.cfdefect.Record.usersRecord" >
	<!--- Place custom code here, it will not be overwritten --->
	<cffunction name="init" access="public" hint="I configure and return this record object." output="false" returntype="any" _returntype="reactor.project.cfdefect.Record.usersRecord">
	...
	<cffunction name="afterDelete" access="private" hint="I am code executed after deleting the record." output="false" returntype="void">
		...
		<cfset deleteRelatedRecords( 'ProjectUsers' ) />
		<!--- delete user projects  email --->
		<cfset deleteRelatedRecords( 'ProjectsUsersEmail' ) />
		<!--- delete user groups --->
		<cfset deleteRelatedRecords( 'userGroups' ) />
		<!--- call super method --->
	...
	<cffunction name="beforeSave" access="private" hint="I am code executed before saving the record." output="false" returntype="void">
		...
		<cfif NOT getFromPreference()>
			...
			<!--- save user projects  --->
			<cfset saveRelatedRecords( 'ProjectUsers', getUserProjects(), 'projectidfk' ) />
		
			<!--- save user projects  email --->
			<cfset saveRelatedRecords( 'ProjectsUsersEmail', filterProjectEmailUsers( getUserProjects(), getUserProjectEmails() ), 'projectidfk' ) />			
			
			<cfif getAdmin()>
				<cfset saveRelatedRecords( 'userGroups', _getReactorFactory().createGateway( 'groups' ).getByFields( name='admin').id, 'groupidfk' ) />
			<cfelse>
				<cfset deleteRelatedRecords( 'userGroups' ) />
			</cfif>
		...
		<cfelse>
			...
			<!--- save user projects  email --->
			<cfset saveRelatedRecords( 'ProjectsUsersEmail', getUserProjectEmails(), 'projectidfk' ) />			
	...
	<cffunction name="afterLoad" access="private" hint="I am code executed after loading the record." output="false" returntype="void">
		<!--- not a new record --->
		<cfif getID() neq '' OR getID() neq 0>
			<cfif ArrayLen( getGroupsIterator().get( name='admin') )>
				<cfset setAdmin( true ) />
			<cfelse>
				<cfset setAdmin( false ) />
			</cfif>
			<cfset setUserProjects( getProjectsIterator().getValueList( 'id' ) ) />
			<cfset setUserProjectEmails( getUsersEmailIterator().getValueList( 'id' ) ) />
		</cfif>
	</cffunction>
