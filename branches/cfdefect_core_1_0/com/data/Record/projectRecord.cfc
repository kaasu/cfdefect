...
<cfcomponent hint="I am the database agnostic custom Record object for the project object.  I am generated, but not overwritten if I exist.  You are safe to edit me."
	extends="reactor.project.cfdefect.Record.projectRecord" >
	<!--- Place custom code here, it will not be overwritten --->
	<!--- Place custom code here, it will not be overwritten --->
	<cffunction name="init" access="public" hint="I configure and return this record object." output="false" returntype="any" _returntype="reactor.project.cfdefect.Record.projectRecord">
	...
	<cffunction name="beforeSave" access="private" hint="I am code executed before saving the record." output="false" returntype="void">
		...
		<!--- save project loci --->
		<cfset saveRelatedRecords( 'projectProjectLocus', getID(), getProjectLociID(), 'projectidfk', 'projectlociidfk' ) />
		<!--- save project users --->
		<cfset saveRelatedRecords( 'projectUser', getID(), getProjectUsersID(), 'projectidfk', 'useridfk' ) />
	...
	<cffunction name="saveRelatedRecords" returntype="void" access="private" output="false" hint="">
		...
		<cfloop list="#arguments.values#" index="i" delimiters="~">
			<cfinvoke component="#_getReactorFactory().createRecord( arguments.name )#" method="init" returnvariable="local.tempRecord">
				<cfinvokeargument name="applicationid" value="#getApplicationID()#" />
	...
	<cffunction name="afterLoad" access="private" hint="I am code executed after loading the record." output="false" returntype="void">
		...
		<cfif getID() neq '' AND getID() neq 0>
			<cfset setProjectLociID( getProjectlocusIterator().getValueList(  'id', '~' ) ) />
			<cfset setProjectUsersID( getUserIterator().getValueList(  'id', '~' ) ) />
		</cfif>
	</cffunction>
	...
	<cffunction name="afterDelete" access="private" hint="I am code executed after deleting the record." output="false" returntype="void">
		...
		<cfset deleteRelatedRecords( 'projectUser', getID(), 'projectidfk' ) />
		<!--- delete project loci --->
		<cfset deleteRelatedRecords( 'projectProjectLocus', getID(), 'projectidfk' ) />
		<!--- delete project announcements --->
		<cfset deleteRelatedRecords( 'announcement', getID(), 'projectidfk' ) />
		<!--- delete project issues --->
		<cfset deleteRelatedRecords( 'issue', getID(), 'projectidfk' ) />
		<!--- call super method --->
