...
<!--- circuit: c --->
<!--- fuseaction: savePreferences --->
<cftry>
<cfset myFusebox.thisPhase = "appinit">
<cfset myFusebox.thisCircuit = "c">
<cfset myFusebox.thisFuseaction = "savePreferences">
...
<cfset Controller.savePreferences( event ) >
<cfif NOT event.getValue( 'recordObject' )._getErrorCollection().hasErrors()>
<cflocation url="#myself#c.preferences&saved=true" addtoken="false">
<cfabort>
<cfelse>
<!--- do action="preferences" --->
<cfset myFusebox.thisFuseaction = "preferences">
<cfset Controller.getUserRecord( event ) >
<cfset xfa.process = "c.savePreferences&id=#event.getValue( 'recordObject' ).getID()#" />
<cfset xfa.cancel = "c.home" />
<cftry>
<cfsavecontent variable="content.pageContent"><cfoutput><cfinclude template="../views/dsp_edit_preferences.cfm"></cfoutput></cfsavecontent>
<cfcatch type="missingInclude"><cfif len(cfcatch.MissingFileName) gte 24 and right(cfcatch.MissingFileName,24) is "dsp_edit_preferences.cfm">
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse dsp_edit_preferences.cfm in circuit v which does not exist (from fuseaction c.preferences).">
