...
<!--- PUBLIC METHODS --->
<cffunction name="getAllRecords" returntype="void" access="public" output="false" hint="">
	<cfargument name="event" type="fusebox5.FuseboxEvent" required="true" hint="" />
	<cfset arguments.event.setValue( 'qAll', getBean( arguments.event.getValue( 'type' ) & 'Service' ).getAllRecords() ) />
...
<cffunction name="getRecord" returntype="void" access="public" output="false" hint="">
	...
	<cfif NOT arguments.event.valueExists( 'recordObject' )>
		<cfset record = getBean( arguments.event.getValue( 'type' ) & 'Service' ).getRecord( arguments.event.getValue( 'id', '0' ) ) />
