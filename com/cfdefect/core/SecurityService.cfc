...
<cffunction name="validateUser" returntype="boolean" access="public" output="false" hint="">
	...
	<cfset var userRecord = getReactorFactory().createRecord( 'users' )>
	<cfset logout() />
	<cfset userRecord.load( username=arguments.username.trim(), password=arguments.password.trim() ) />
...
<cffunction name="paramUser" returntype="void" access="private" output="false" hint="">
	...
	<cfif not getSessionFacade().exists( 'user' )>
		<cfset user = getReactorFactory().createRecord( 'users' ) />
