...
<cffunction name="getProjectForUser" returntype="query" access="public" output="false" hint="I return all projects for a user.">
	<cfargument name="user_id" type="string" required="true" hint="" />
	<!--- TODO: Modify this to use a query instead --->
	<cfreturn getRecord( id = arguments.user_id ).getProjectIterator().getQuery() /> 
</cffunction>

<cffunction name="getUserProjectStats" returntype="struct" access="public" output="false" hint="I return accumlated status of projects and issue assigned to a user.">
	<cfargument name="useridfk" type="string" required="false" hint="" />
	<cfset var issues = getIssueService().getIssuesForProjectUser( 	applicationid=getApplicationId(),
																	useridfk=arguments.useridfk )>
